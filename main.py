...
Fetches current USD price + 24h % change for BTC, ETH, BNB, PI, TON
from the CoinGecko public API and emails a summary via SMTP.
...
def fetch_prices() -> dict:
    """Fetch current price + 24h change for all coins in one request."""
    params = {
        "ids": ",".join(COINS.keys()),
        "vs_currencies": "usd",
...
def build_email_body(data: dict) -> tuple[str, str]:
    ...
    for coin_id, label in COINS.items():
        ...
        if not coin_data:
            ...
            html_rows.append(
                f"<tr><td>{label}</td><td colspan='2'>data unavailable</td></tr>"
            )
            continue
        ...
        text_lines.append(f"{label}: {price_str}  ({change_str} 24h)")
        html_rows.append(
            f"<tr>"
            f"<td style='padding:8px 12px;border-bottom:1px solid #eee;'>{label}</td>"
            f"<td style='padding:8px 12px;border-bottom:1px solid #eee;text-align:right;'>{price_str}</td>"
